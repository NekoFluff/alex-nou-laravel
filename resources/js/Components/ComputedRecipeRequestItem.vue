...
<template>
    <li class="flex flex-row p-3 mb-2 rounded-md hover:bg-gray-200">
        <button class="flex flex-row items-center w-full" @click="$emit('click', recipeRequest)">
            <img
                v-if="recipesStore.recipeMap[props.recipeRequest.name][0].image"
                class="inline w-6 h-6"
                :src="recipesStore.recipeMap[props.recipeRequest.name][0].image"
                :alt="props.recipeRequest.name"
            />
